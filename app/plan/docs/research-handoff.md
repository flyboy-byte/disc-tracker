...
| - | -------- | ---------- | ----------------- | ---------------- |
| ~~1~~ | ~~What's the current, realistic state of F-Droid's reproducible-build acceptance for Expo/React Native apps built with local Gradle (not EAS)...~~ | `approach.md`, `risks.md` | — | **Resolved 2026-07-23 without external research** — the developer's other Expo/RN app (DragTree) already achieved a full `Binaries:` byte-match and got merged into F-Droid's official index (MR #41671). Real, primary-source answer now lives in `fdroid-reference.md`, distilled from that submission. No deep-research handoff needed. |

...
1. **A live Flask web app** — personal disc golf bag tracker running on a VPS at `51.81.80.126`. Multi-user, local SQLite, no cloud, no accounts. **The website remains the canonical spec** — the reference implementation new behavior gets designed against, and what "the two disagree, the website wins" still means throughout this doc. That's a distinct claim from which one Logan actually *uses* day to day (see point 2) — spec-of-record and primary-deployment-target are two different roles, and only the first is fixed.

2. **An Android app port — v1 feature-complete, actively growing, and increasingly Logan's primary deployment** — Expo (React Native) app, local-first SQLite, targeting Play Store + F-Droid. Plan docs are in `app/`. As of 2026-08-21, Logan is "moving more towards the app as main deployment" (still progressing, not a cutover — the website stays the spec per point 1, this is about which one he's actually building toward and using) — see `PLAN.md` Track D sub-track 3's note and `app/plan/docs/track-d-flight-arc-parity.md` for where this first came up. Now **five tabs** (Bag / Flight Shaper / Disc Suggest / **Score** / Settings), all built and verified, first shipped as `mobile-preview-0.5` and **confirmed on a real physical Android phone (2026-07-24)**. Latest tagged release is **`v0.25`** (2026-08-20: Disc Suggest swipe hint text + stronger
...
- **R7 F-Droid (official index, D3)** — **not dropped** (it's the FOSS destination), just
...
- Both R6 and R7-the-official-index submission are **deliberately parked** — Logan's
  ...
  opening a public store listing or a reviewer relationship — it's Logan's own repo.
